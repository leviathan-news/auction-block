...
def create_new_auction(ipfs_hash: String[46] = "") -> uint256:
    ...
    """
    pausable._require_not_paused()
    if msg.sender != ownable_base.owner:
...
def nullify_auction(auction_id: uint256):
    ...
    log AuctionNullified(auction_id=auction_id)
...
def set_fee_receiver(_fee_receiver: address):
    ...
    log FeeReceiverUpdated(fee_receiver=_fee_receiver)
...
def set_fee_percent(_fee: uint256):
    ...
    log FeePercentUpdated(fee_percent=_fee)
...
def set_approved_directory(directory_address: address):
    ...
    log DirectorySet(directory_address=directory_address)
