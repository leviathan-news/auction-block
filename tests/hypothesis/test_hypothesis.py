...
weth_amounts = st.decimals(min_value=Decimal("0.001"), max_value=Decimal("9.9"), places=18).map(
    lambda d: int(d * 10**18)
)
...
@settings(deadline=None, max_examples=25)  # Limit test cases since each fork test is slow
            ...
            if not approve_weth:
                ...
                with pytest.raises(Exception) as e:
                    auction_house.create_bid_with_token(
                        auction_id, weth_amount, weth, min_output_with_slippage, bidder
                    )
                ...
                ), f"Spent more WETH than approved: {weth_spent} > {weth_amount}"

                if auction[4] == bidder:  # If winning bid
                    assert (
                        auction[1] >= min_output_with_slippage
                    ), "Output should meet minimum with slippage"
...
@pytest.mark.fork_only
def test_edge_case_scenarios(auction_house, weth_trader, weth, payment_token, make_user):
    ...
    with boa.env.prank(bidder):
        ...
        auction_house.create_bid_with_token(
            auction_id, min_weth, weth, min_output_with_slippage, bidder
        )
    ...
    with boa.env.prank(next_bidder):
        weth.approve(auction_house, 2**256 - 1)
        auction_house.create_bid_with_token(
            auction_id, exact_increment_weth, weth, min_output_with_slippage, next_bidder
        )
