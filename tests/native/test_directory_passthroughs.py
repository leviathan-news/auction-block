...
def test_directory_overbid_possible(
    ...
    assert (
        directory_balance_before == directory_balance_after
    ), "Directory's balance should not change"
...
def test_withdraw_permission_check(
    ...
    assert (
        pending_returns_after == pending_returns
    ), f"Alice's {pending_returns / 10 ** 18} returns were withdrawn with incorrect permissions"


def test_withdraw_multiple_permission_check(
    auction_house_with_multiple_auctions,
    alice,
    bob,
    charlie,
    payment_token,
    ...
    for auction_id in auction_ids:
        ...
        assert (
            pending_after > 0
        ), f"Alice's returns for auction {auction_id} were withdrawn with incorrect permissions"
...
def test_successful_overbid_through_directory(
    ...
    assert (
        balance_change == overbid_amount
    ), f"Expected {overbid_amount} tokens transferred, but got {balance_change}"
    ...
    assert (
        additional_transfer == expected_transfer
    ), f"Expected {expected_transfer} additional tokens transferred, but got {additional_transfer}"
...
def test_create_bid_with_token_prevents_increasing_own_bid(
    ...
    with boa.env.prank(alice):
        ...
        directory.create_bid_with_token(
            house, auction_id, initial_weth_amount, weth, expected_payment, ""
        )
    ...
    alice_init = weth.balanceOf(alice)
...
def test_create_bid_with_token_allows_increasing_own_bid(
    ...
    if initial_bid_successful:
        ...
        assert final_bid == current_bid + expected_additional_payment
        assert weth.balanceOf(alice) == alice_weth_before - additional_weth
