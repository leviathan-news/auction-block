...
def load_supported_tokens(directory):
    ret_arr = []
    for i in range(10):
        try:
            ret_arr.append(directory.supported_tokens(i))
...
def test_add_token_support(directory, deployer, payment_token, inert_weth, inert_weth_trader):
    weth = inert_weth
    weth_trader = inert_weth_trader

    with boa.env.prank(deployer):
        # Initially, no supported tokens
        initial_tokens = load_supported_tokens(directory)
        ...
        tokens_after_first = load_supported_tokens(directory)
        ...
        directory.add_token_support(test_token, boa.env.generate_address())

        # Check supported tokens after second addition
        tokens_after_second = load_supported_tokens(directory)
...
def test_cannot_add_payment_token(directory, payment_token, deployer):

    with boa.env.prank(deployer), boa.reverts("!payment_token"):
        directory.add_token_support(payment_token, boa.env.generate_address())


def test_revoke_token_support(directory, deployer, payment_token, inert_weth, inert_weth_trader):
    weth_token = boa.load_partial("contracts/test/ERC20.vy").deploy("Wrapped ETH", "WETH", 18)
    weth_trader = boa.env.generate_address()

    with boa.env.prank(deployer):
        # Add multiple tokens
        directory.add_token_support(weth_token, weth_trader)
        ...
        # Initial state check
        tokens_before = load_supported_tokens(directory)
        ...
        directory.revoke_token_support(weth_token)

        # Check supported tokens after first removal
        tokens_after_first_removal = load_supported_tokens(directory)
        assert len(tokens_after_first_removal) == 1
        assert tokens_after_first_removal[0] == test_token.address

        # Revoke the remaining token
        directory.revoke_token_support(test_token)

        # Check supported tokens after second removal
        tokens_after_second_removal = load_supported_tokens(directory)
        assert len(tokens_after_second_removal) == 0


def test_revoke_token_support_order_preservation(directory, deployer, payment_token):
    with boa.env.prank(deployer):
        # Deploy test tokens
        weth_token = boa.load_partial("contracts/test/ERC20.vy").deploy("Wrapped ETH", "WETH", 18)
        ...
        # Add tokens
        directory.add_token_support(weth_token, boa.env.generate_address())
        directory.add_token_support(test_token_1, boa.env.generate_address())
        directory.add_token_support(test_token_2, boa.env.generate_address())
        ...
        directory.revoke_token_support(test_token_1)

        # Check order preservation
        tokens_after_removal = load_supported_tokens(directory)
