...
def test_mint_works_without_nft(auction_house_with_auction, alice, default_reserve_price, deployer, nft, payment_token, zero_address):
...
def test_nft_mints_on_complete_auction(auction_house_with_auction, alice, default_reserve_price, deployer, nft, payment_token, base_uri_prefix):
    house = auction_house_with_auction
    with boa.env.prank(deployer):
        house.set_nft(nft)
    auction_id = house.auction_id()
...
def test_nft_id_matches_auction_id(auction_house_with_auction, alice, default_reserve_price, deployer, nft, payment_token):
    house = auction_house_with_auction
    with boa.env.prank(deployer):
        house.set_nft(nft)
    auction_id = house.auction_id()
