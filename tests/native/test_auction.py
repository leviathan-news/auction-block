...
def test_create_bid(
    ...
    assert auction[1] == default_reserve_price, f"Expected amount to be {default_reserve_price}, got {auction[1]}"
    assert payment_token.balanceOf(house.address) == default_reserve_price, "Expected house to hold tokens"
...
def test_outbid(
    ...
    min_next_bid = default_reserve_price + (default_reserve_price * house.min_bid_increment_percentage() // 100)
    print(f"Minimum next bid required: {min_next_bid}")
    
    # Try insufficient bid
    insufficient_bid = default_reserve_price + 1
    ...
    assert payment_token.balanceOf(house.address) == expected_balance, \
